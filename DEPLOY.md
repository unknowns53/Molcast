...
dry-run で挙動をプレビューしたい場合は `--policy=cleanup-policy.json --dry-run` を付けて呼ぶと、削除を抑止したままポリシーが登録される。Phase 3 で OPSIN (JRE + py2opsin) を復活させてイメージが 200 MB 台に増えても、本ポリシーが効いていれば 0.5 GB 無料枠内に収まる想定。
...
→ `name: 経路は Phase 3 で対応予定です。...` (Phase 1 では意図どおりの挙動)。
