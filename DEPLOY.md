...
→ 3 件とも `app/opsin_aliases.json` の慣用名 mapping で即解決され、それぞれビューア URL が返る (alias hit は `subprocess` 起動せず、構造化ログに `opsin_alias_hit` が記録される)。

```
/mol name: hexafluorobenzene
```

→ alias 無いので OPSIN local backend で解決される (`opsin_resolved` ログに `backend: local`)。初回 cold start ではここで JVM 起動コスト ~1-2 秒が乗るので、Slack の 3 秒 ack を超えるとユーザ側に「アプリが応答しなかった」が出る。warm 状態で再投入すれば通る。

```
/mol name: not_a_real_compound_xyz
```

→ `OPSIN は体系名のみ対応です。慣用名・商品名は解釈できません。SMILES を直接入力してください: /mol <SMILES>` が ephemeral で返る。
